---
title: Perception
description: AprilTag detection with OpenCV ArUco and live-CameraInfo solvePnP, a YOLO gripper-cam flower grader with an HSV sim stand-in, and an aggregator that TF-projects tags into map, debounces a discovered-tag registry, and tag-anchors each bloom into a planter box for the mission, twin, and HMI.
---

Lupin's perception stack reads the greenhouse two ways: it locates **AprilTag climate stations** from the Orbbec RGB stream, and it **classifies tulips and pest anomalies** from the gripper camera. An aggregator then fuses both into one map-localised, debounced **registry** that the [mission orchestrator](/docs/mission) and the [digital twin](/docs/digital-twin) consume, and it places every detected bloom into the planter box around each tag instead of stacking dots on the marker.

This page is the reference for that fusion: the two detectors, the sim-versus-hardware swap point, the tag-anchored box geometry that turns a 4 cm marker into a bed of located flowers, and the temporal-co-location rule that stands in for a shared 3-D frame Lupin does not have.
...
### Tag-anchored bloom placement (Stream B)

This is the cleverest idea in the stack, and it used to be invisible. A single 4 cm tag plus a **known box size** fully determines every bloom's map position *without depth*, because the box is a rigid transform of the tag pose. `box_geometry.py` owns that math as a pure, ROS-free, unit-tested module (it mirrors how `lupin_mission/approach.py` isolates the approach-pose geometry).

The frame is built from the tag's map pose:

- **normal `n`**, the tag's local +Z axis projected onto the map XY plane, `nx = 2(qx*qz + qw*qy)`, `ny = 2(qy*qz - qw*qx)`, the *same* derivation the approach-pose code uses. It points out of the bed into the aisle, toward the camera that read the tag.
- **lateral `l`**, `n` rotated +90 degrees in XY, i.e. `(-n_y, n_x)`, the across-the-bed axis.
- **interior**, extends *opposite* `n` (into the bed) by `depth` and spans `+/- width/2` along `l`, with an optional `tag_lateral_offset`. A bloom at lateral fraction `f in [-1, 1]` and depth fraction `d in [0, 1]` maps to `origin + l*(f*width/2) - n*(d*depth)`. The `StandardBox` defaults are `width=0.80`, `depth=0.40`.
...
<Mermaid chart={`graph TD
  POSE["tag pose in map"] --> NORM["tag_normal_xy<br/>+Z projected to XY"]
  NORM --> FRAME["box frame<br/>origin, normal, lateral"]
  ...
  FRAME --> PLACE["geom.place(f, d)"]
  BIN --> PLACE
  PLACE --> OUT["FlowerPoint[] + box_footprint"]
  NORM -->|"degenerate normal"| FALL["single dot on tag pose"]
  FALL --> OUT
`} />

<Callout type="info">
  **Degenerate-normal fallback.** When the tag faces straight up or down, or its orientation is too noisy to derive an in-plane normal, `box_from_tag` returns `None` and `bin_detections` returns `[]`. In that case `_emit_flower_observation` falls back to a single bloom **at the tag pose**, so a classified or pest-flagged tag still shows one dot. This fixed a real bug where a `bug`-only degenerate tag emitted no point at all.
</Callout>
