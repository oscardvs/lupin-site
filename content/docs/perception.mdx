...
- **Place the rectangle.** For each tag, `nearest_table_rect` picks its bench, the four corners are transformed into the map frame, and `box_geometry_from_corners` builds the box frame: the bench's **long edge is the lateral axis `l`**, its **short edge the depth/normal `n`** (signed toward the tag so the box *front* faces the aisle), with the origin at the centre of that front edge. A bloom at lateral fraction `f in [-1, 1]` and depth fraction `d in [0, 1]` maps to `origin + l*(f*width/2) - n*(d*depth)`, exactly the placement the rest of Stream B already used. Drawing the *full* rectangle is also what "fills in" the cells the lidar can't see behind the near face.

When the layout can't be resolved (package missing, or a tag with no table) the node **falls back** to the legacy box: a fixed `StandardBox` (fallback `width=0.80`, `depth=0.40`) hung off the single tag pose, with the in-plane normal `n` taken from the tag quaternion, `nx = 2(qx*qz + qw*qy)`, `ny = 2(qy*qz - qw*qx)`. That legacy path produced the old, wrong-sized overlay (a `0.80 x 0.40` box per *tag*, so ~4 overlapping rects per bench); it now runs only when the real rectangle is unavailable.
...
<Mermaid chart={`graph TD
  TAGS["discovered tags<br/>map poses"] --> REG["solve_rigid_2d<br/>JSON to map (Kabsch)"]
  JSON["tag_locations.json<br/>table rectangles"] --> REG
  JSON --> NT["nearest_table_rect"]
  REG --> XF["transform corners<br/>to map frame"]
  NT --> XF
  XF --> FRAME["box frame<br/>long=lateral, short=normal"]
