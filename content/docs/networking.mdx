...
`post-boot-sync.sh` encodes the safe procedure: it only steps the clock when the drift exceeds 3 s, and only with the stack **stopped**, then wipes SHM, then restarts. Re-running it mid-session as a health check is a no-op on the clock.
